var WiredElements = (function (exports) {
  ...
  */
  ...
  class TemplateResult$1 {
      constructor(strings, values, type, partCallback = defaultPartCallback$1) {
          this.strings = strings;
          this.values = values;
          this.type = type;
          this.partCallback = partCallback;
      ...
      getHTML() {
          ...
          for (let i = 0; i < l; i++) {
              ...
              // If it doesn't have any tags, then we use the previous text position
              // state.
              const closing = findTagClose$1(s);
              isTextBinding = closing > -1 ? closing < s.length : isTextBinding;
   ...
   * result.type and result.strings.
   */
  function defaultTemplateFactory$1(result) {
   ...
   *     cache.
   */
  function render$2(result, container, templateFactory = defaultTemplateFactory$1) {
      const template = templateFactory(result);
      let instance = container.__templateInstance;
      // Repeat render, just call update()
      if (instance !== undefined && instance.template === template &&
      ...
      // First render, create a new TemplateInstance and append it
      instance =
          new TemplateInstance$1(template, result.partCallback, templateFactory);
      container.__templateInstance = instance;
      const fragment = instance._clone();
      instance.update(result.values);
          ...
          while (walker.nextNode()) {
              ...
              if (node.nodeType === 1 /* Node.ELEMENT_NODE */) {
                  ...
                  while (count-- > 0) {
                      ...
                      const stringForPart = result.strings[partIndex];
                      // Find the attribute name
                      const attributeNameInPart = lastAttributeNameRegex$1.exec(stringForPart)[1];
                      // Find the corresponding attribute
                      const attribute = attributes.getNamedItem(attributeNameInPart);
                      const stringsForAttributeValue = attribute.value.split(markerRegex$1);
                      this.parts.push(new TemplatePart$1('attribute', index, attribute.name, attributeNameInPart, stringsForAttributeValue));
                      node.removeAttribute(attribute.name);
                      partIndex += stringsForAttributeValue.length - 1;
                  }
              }
  ...
  const getValue$1 = (part, value) => {
      ...
      if (isDirective$1(value)) {
          value = value(part);
          return directiveValue$1;
      }
      return value === null ? undefined : value;
  };
  const isDirective$1 = (o) => typeof o === 'function' && o.__litDirective === true;
  /**
   * A sentinel value that signals that a value was handled by a directive and
   * should not be written to the DOM.
   */
  const directiveValue$1 = {};
  const isPrimitiveValue$1 = (value) => value === null ||
      !(typeof value === 'object' || typeof value === 'function');
  ...
  class NodePart$1 {
      ...
      _setTemplateResult(value) {
          ...
          if (this._previousValue && this._previousValue.template === template) {
              ...
              instance = new TemplateInstance$1(template, this.instance._partCallback, this.instance._getTemplate);
              this._setNode(instance._clone());
              this._previousValue = instance;
          }
          instance.update(value.values);
      ...
      _setIterable(value) {
          ...
          for (const item of value) {
              ...
              if (itemPart === undefined) {
                  ...
                  itemPart = new NodePart$1(this.instance, itemStart, this.endNode);
                  itemParts.push(itemPart);
              }
              itemPart.setValue(item);
              partIndex++;
  ...
  const defaultPartCallback$1 = (instance, templatePart, node) => {
      if (templatePart.type === 'attribute') {
          return new AttributePart$1(instance, node, templatePart.name, templatePart.strings);
      }
      else if (templatePart.type === 'node') {
          return new NodePart$1(instance, node, node.nextSibling);
      }
      throw new Error(`Unknown part type ${templatePart.type}`);
  };
  /**
  ...
  function render$3(result, container, scopeName) {
      return render$2(result, container, shadyTemplateFactory$1(scopeName));
  }
   ...
   * Interprets a template literal as a lit-extended HTML template.
   */
  const html$3 = (strings, ...values) => new TemplateResult$1(strings, values, 'html', extendedPartCallback$1);
  /**
   * A PartCallback which allows templates to set properties and declarative
   * event handlers.
   *
   ...
   *
   */
  const extendedPartCallback$1 = (instance, templatePart, node) => {
      if (templatePart.type === 'attribute') {
          if (templatePart.rawName.startsWith('on-')) {
              const eventName = templatePart.rawName.slice(3);
              return new EventPart$1(instance, node, eventName);
          }
          if (templatePart.name.endsWith('$')) {
              const name = templatePart.name.slice(0, -1);
              return new AttributePart$1(instance, node, name, templatePart.strings);
          }
          if (templatePart.name.endsWith('?')) {
              const name = templatePart.name.slice(0, -1);
              return new BooleanAttributePart$1(instance, node, name, templatePart.strings);
          }
          return new PropertyPart$1(instance, node, templatePart.rawName, templatePart.strings);
      }
      return defaultPartCallback$1(instance, templatePart, node);
  };
  /**
   * Implements a boolean attribute, roughly as defined in the HTML
   * specification.
  ...
  class LitElement$1 extends PropertiesMixin(HTMLElement) {
      constructor() {
          super(...arguments);
          this.__renderComplete = null;
          this.__resolveRenderComplete = null;
       ...
       */
      _applyRender(result, node) {
          render$3(result, node, this.localName);
      }
      /**
       * Called after element DOM has been rendered. Implement to
       * directly control rendered DOM. Typically this is not needed as `lit-html`
  ...
  */
  ...
  class Icon extends LitElement$1 {
    _renderStyle() {
      return style;
    }
    _render() {
      return html$3`${this._renderStyle()}<slot></slot>`;
    }
  }
